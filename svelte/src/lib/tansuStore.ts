/* eslint-disable */
// tweaked version of https://amadeusitgroup.github.io/tansu/tansu.html
...
const bind: <T>(object: T | null | undefined, fnName: keyof T) => any = <T>(
  object: T | null | undefined,
  fnName: keyof T
): any => {
  const fn: T[keyof T] | null = object ? object[fnName] : null;
  return typeof fn === 'function' ? fn.bind(object) : noop;
};

const toSubscriberObject: <T>(
  subscriber: Subscriber<T>
) => PrivateSubscriberObject<T> = <T>(
  subscriber: Subscriber<T>
...
const returnThis: <T>(this: T) => T = function <T>(this: T): T {
  return this;
};

const asReadable: <T>(store: Store<T>) => Readable<T> = <T>(
  store: Store<T>
): Readable<T> => ({
...
const queueProcess: unique symbol = Symbol();
let willProcessQueue = false;
const queue: Set<{ [queueProcess](): void }> = new Set<{
  [queueProcess](): void;
}>();

const callUnsubscribe: (unsubscribe: Unsubscriber) => void = (
  unsubscribe: Unsubscriber
): void =>
  typeof unsubscribe === 'function' ? unsubscribe() : unsubscribe.unsubscribe();

function notEqual(a: any, b: any): boolean {
  const tOfA: any = typeof a;
 ...
 * ```
 */
export const batch: <T>(fn: () => T) => T = <T>(fn: () => T): T => {
  const needsProcessQueue: boolean = !willProcessQueue;
...
export abstract class Store<T> implements Readable<T> {
  private _subscribers: Set<PrivateSubscriberObject<T>> = new Set<
    PrivateSubscriberObject<T>
  >();
  private _cleanupFn: null | Unsubscriber = null;
  private _subscribersPaused = false;
  private _valueIndex: number = 1;
  ...
  private _stop(): void {
    const cleanupFn: UnsubscribeObject | (() => void) | null = this._cleanupFn;
    if (cleanupFn) {
  ...
  private [queueProcess](): void {
    ...
    const valueIndex: number = this._valueIndex;
    const value: T = this._value;
    for (const subscriber of [...this._subscribers]) {
      if (
        subscriber._valueIndex !== valueIndex &&
        notEqual(subscriber._value, value)
      ) {
        subscriber._valueIndex = valueIndex;
  ...
  subscribe(
    ...
    const subscriberObject: PrivateSubscriberObject<T> =
      toSubscriberObject(subscriber);
    ...
    const unsubscribe: UnsubscribeFunction & UnsubscribeObject = (): void => {
      const removed: boolean = this._subscribers.delete(subscriberObject);
...
const noopUnsubscribe: UnsubscribeFunction & UnsubscribeObject = (): void => {};
...
export function readable<T>(
  ...
  const ReadableStoreWithOnUse: any = class extends Store<T> {
    protected onUse(): void | UnsubscribeObject | (() => void) {
      const setFn: any = (v: T): void => this.set(v);
      setFn.set = setFn;
      setFn.update = (updater: Updater<T>): void => this.update(updater);
...
export function writable<T>(
  ...
  const WritableStoreWithOnUse: any = class extends WritableStore<T> {
    protected onUse(): void | UnsubscribeObject | (() => void) {
      const setFn: any = (v: T): void => this.set(v);
      setFn.set = setFn;
      setFn.update = (updater: Updater<T>): void => this.update(updater);
  ...
  const store: any = new WritableStoreWithOnUse(value);
  return {
    ...asReadable(store),
...
> extends Store<T> {
  ...
  protected onUse(): Unsubscriber | void {
    ...
    const stores: S = this._stores;
    const isArray: boolean = Array.isArray(stores);
    const storesArr: readonly SubscribableStore<any>[] = isArray
      ? (stores as readonly SubscribableStore<any>[])
      : [stores as SubscribableStore<any>];
    const dependantValues: any[] = new Array(storesArr.length);

    let cleanupFn: null | Unsubscriber = null;

    const callCleanup: () => void = (): void => {
      const fn: UnsubscribeObject | (() => void) | null = cleanupFn;
      if (fn) {
        cleanupFn = null;
        callUnsubscribe(fn);
      }
    ...
    const callDerive: () => void = (): void => {
      if (initDone && !pending) {
        if (changed) {
          changed = 0;
          callCleanup();
          cleanupFn =
            this.derive(isArray ? dependantValues : dependantValues[0]) || noop;
        }
        this.resumeSubscribers();
      }
    };

    const unsubscribers: (UnsubscribeObject | (() => void))[] = storesArr.map(
      (
        store: SubscribableStore<any>,
        idx: number
      ): UnsubscribeObject | (() => void) =>
        store.subscribe({
          next: (v: any): void => {
...
export function derived<T, S extends SubscribableStores>(
  ...
  const Derived: any = isSyncDeriveFn(deriveFn)
    ? class extends DerivedStore<T, S> {
        protected derive(values: SubscribableStoresValues<S>): void {
          this.set(deriveFn(values));
        }
      }
    : class extends DerivedStore<T, S> {
        protected derive(values: SubscribableStoresValues<S>): any {
          const setFn: any = (v: T): void => this.set(v);
          setFn.set = setFn;
          setFn.update = (updater: Updater<T>): void => this.update(updater);
